...
def parse_args():
    ...
    parser.add_argument("--resume_pth", type=str, default=None,
                        help="specify a .pth path to resume training, or None to train from scratch")
    parser.add_argument("--data_root", type=str, default="OPA", help="dataset root")
    parser.add_argument("--eval_type", type=str, choices=["train", "trainpos", "sample", "eval", "evaluni"],
                        default="eval", help="evaluation type")
    ...
    parser.add_argument("--with_infer", action='store_true', default=False,
                        help="action to make inference after each training epoch")
    parser.add_argument("--encoder_type", type=str, choices=["resnet", "pyresnet", "convnext", "vit"], default="resnet",
                        help="Encoder type for Sysnet")
...
def main():
    ...
    model_dir, sample_dir, tblog_dir, log_path = dirs['model_dir'], dirs['sample_dir'], dirs['tblog_dir'], dirs[
        'log_path']
    ...
    train_loader = get_loader(opt.dst, batch_size=opt.batch_size, num_workers=8, image_size=opt.img_size, shuffle=True,
                              mode_type="train", data_root=opt.data_root)
    sample_dataset = get_dataset(opt.dst, image_size=opt.img_size, mode_type="sample", data_root=opt.data_root)
    eval_loader = get_loader(opt.dst, batch_size=1, num_workers=1, image_size=opt.img_size, shuffle=False,
                             mode_type=opt.eval_type, data_root=opt.data_root)
    
    model = GAN(opt)
    model, start_ep = resume(opt.resume_pth, model, resume_list=['generator', 'discriminator'], strict=True,
                             logger=logger)
    ...
    for epoch in range(start_ep, opt.n_epochs):
        for i, (indices, bg_img_feats, fg_img_feats, fg_msk_feats, comp_img_feats, comp_msk_feats, comp_bboxes, labels,
                catnms) in enumerate(train_loader):
            model.start_train()
            g_gan_loss, g_ndiv_loss, d_real_loss, d_fake_loss = model.train_disc_gen(bg_img_feats, fg_img_feats,
                                                                                     fg_msk_feats, comp_img_feats,
                                                                                     comp_msk_feats, comp_bboxes,
                                                                                     labels)
            ...
            if (epoch * len(train_loader) + i) % 10 == 0:
                logger.info(
                        ...
                        % (
                            epoch + 1, opt.n_epochs, i + 1, len(train_loader), g_gan_loss_meter.avg,
                            g_ndiv_loss_meter.avg,
                            d_real_loss_meter.avg, d_fake_loss_meter.avg)
                )
