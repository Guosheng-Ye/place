...
from network import Encoder, Encoder_pyconv_18, Encoder_convnext
from vit_model import vit_for_image_composite
...
class SysNet(nn.Module):
    """
    Generator()
    system net have fg_encoder,bg_encoder,decoder
    :return
    noise:[8,3]
    locations:[8,3]
    fg_feats:[8,512]
    ...
    def __init__(self, opt, init_weight=True):
        ...
        if opt.encoder_type == "pyresnet":
            self.fg_encoder = Encoder_pyconv_18()  # [2,512]
            self.bg_encoder = Encoder_pyconv_18()  # [2,512]
        if opt.encoder_type == "resnet":
            self.bg_encoder = Encoder()
            self.fg_encoder = Encoder()
        if opt.encoder_type == "convnext":
            self.fg_encoder = Encoder_convnext()
            self.bg_encoder = Encoder_convnext()
        if opt.encoder_type == "vit":
            self.fg_encoder = Encoder()
            self.bg_encoder = vit_for_image_composite()
        
        self.decoder = nn.Sequential(
                nn.Linear(opt.d_emb * 2 + 3, opt.d_fc_gen),  # 512 * 2 + 3 = 1027
                nn.BatchNorm1d(opt.d_fc_gen),
                nn.ReLU(inplace=True),
                
                nn.Linear(opt.d_fc_gen, opt.d_fc_gen),  # 512 -> 512
                nn.BatchNorm1d(opt.d_fc_gen),
                nn.ReLU(inplace=True),
                nn.Linear(opt.d_fc_gen, 3),  # 512 -> 3
                nn.Sigmoid()
        )  # [1024->3]
    ...
    def initialize_weight(self):
        """
        apply to initialize weight
        """
        self.decoder.apply(weights_init_normal)
    
    def enc(self, bg_img, fg_img, samp_N):
        """
        encoder the fg_img and bg_img
        :param bg_img:input_size:[2,3,256,256]
        :param fg_img:input_size:[2,3,256,256]
        :param samp_N:
        :return:fg_feats:[8,512]
                bg_feats:[8,512]
        """
        ...
        # [2,512] -> [2,1,512] -> [2,4,512]
        fg_feats = fg_feats_.view(-1, fg_feats_.shape[2])  # [8,512]
        bg_feats = bg_feats_.view(-1, bg_feats_.shape[2])  # [8,512]
        return fg_feats, bg_feats  # [8,512] [8,512]
    
    def dec(self, noises, fg_feats, bg_feats):
        """
        
        :param noises:[8,3]
        :param fg_feats:[8,512]
        :param bg_feats:[8,512]
        :return: locations:[8,3]
                 decoder(cat(fg_feats,bg_feats,noise))
        """
        dec_in = torch.cat((fg_feats, noises, bg_feats), dim=1)  # 512 + 512 + 3
        locations = self.decoder(dec_in)  # 512 + 512 + 3 -> 3
        return locations
    ...
    def forward(self, bg_img, fg_img, is_train=True):
        ...
        fg_feats, bg_feats = self.enc(bg_img_norm, fg_img_norm, samp_N=samp_N)  # [8,512] [8,512]
        noises = torch.randn((batch_size * samp_N, 3)).cuda()  # (2*4,3)
...
class Discriminator(nn.Module):
    ...
    def __init__(self, opt, init_weight=True):
        ...
        self.model = nn.Sequential(
                nn.Linear(opt.d_emb * 2 + 3, opt.d_fc_disc),  # 1027 -> 512
                nn.BatchNorm1d(opt.d_fc_disc),
                nn.LeakyReLU(negative_slope=0.2, inplace=True),
                nn.Linear(opt.d_fc_disc, opt.d_fc_disc),  # 512 -> 512
                nn.BatchNorm1d(opt.d_fc_disc),
                nn.LeakyReLU(negative_slope=0.2, inplace=True),
                nn.Linear(opt.d_fc_disc, 1),  # 512 -> 1
                nn.Sigmoid()
        )
    ...
    def forward(self, locations, fg_feats, bg_feats):
        ...
        :param fg_feats: [8,512]
        :param bg_feats: [8,512]
        :return:1-d:[8] self.model(cat([fg_feats,bg_feats,locations])).view(-1)
        """
        disc_in = torch.cat([fg_feats, locations, bg_feats], dim=1)  # [8,1027] ->[8,1]
        disc_out = self.model(disc_in)  # [8,1027] -> [8,1]
        return disc_out.view(-1)  # [8]
...
class GAN(object):
    ...
    def start_train(self):
        """
        to train mode to use batch_normalization and drop out
        """
    ...
    def state_dict(self):
        """
        state dict for generator and discriminator
        :return:
        """
        model_dict = dict()
    ...
    def train_disc_gen(self, bg_img, fg_img, fg_msk, comp_img, comp_msk, comp_bbox, label):
        """
        train for discriminator and generator
        :param bg_img:
        :param fg_img:
        :param fg_msk:
        :param comp_img:
        ...
        comp_bbox_v = Variable(comp_bbox, requires_grad=False).cuda().unsqueeze(1).repeat(1, self.opt.samp_N, 1).view(
                -1, 3)
        ...
        g_ndiv_loss = torch.sum(F.relu(self.opt.margin * D_z_avg - D_y_avg)) / (
                (self.opt.samp_N * self.opt.samp_N - self.opt.samp_N) * batch_size)
