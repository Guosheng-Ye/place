...
def gen_composite_image(bg_img, fg_img, fg_msk, trans, fg_bbox=None):
    ...
    start_x, start_y, width, height = round((bg_w - fg_w_new) * relative_x), round(
        (bg_h - fg_h_new) * relative_y), round(fg_w_new), round(fg_h_new)
    ...
    comp_img_arr, bg_img_arr, fg_img_arr, fg_msk_arr = np.array(bg_img), np.array(bg_img), np.array(
        fg_img_new), np.array(fg_msk_new)
    fg_msk_arr_norm = fg_msk_arr[:, :, np.newaxis].repeat(3, axis=2) / 255.0
    comp_img_arr[start_y:start_y + height, start_x:start_x + width, :] = fg_msk_arr_norm * fg_img_arr + (
                1.0 - fg_msk_arr_norm) * bg_img_arr[start_y:start_y + height, start_x:start_x + width, :]
