...
class OPADst1(OPABasicDataset):
    ...
    def __getitem__(self, index):
        index_, annid, scid, bbox, scale, label, catnm, bg_img, fg_img, fg_msk, comp_img, comp_msk = super().__getitem__(
                index)
    ...
    def img_trans_bg(self, x):
        """
        resize to self. Size and to Tensor
        img_size:256
        :param x:
        :return:
        """
    ...
    def img_trans_fg(self, x, x_mode, bg_img, fg_img):
        ...
        if bg_w / bg_h > fg_w / fg_h:
            y = transforms.Resize((self.size, (self.size * fg_w * bg_h) // (fg_h * bg_w)),
                                  interpolation=Image.BILINEAR)(x)
            ...
            y = transforms.Resize(((self.size * fg_h * bg_w) // (fg_w * bg_h), self.size),
                                  interpolation=Image.BILINEAR)(x)
